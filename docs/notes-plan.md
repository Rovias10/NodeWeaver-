...
> ⚠️ **Estado del documento (post rama `IA_Integration`).** Este plan describe el **diseño previo** y se redactó cuando el proveedor IA elegido era **Ollama + Smalot/PDFParser**. La rama `IA_Integration` cambió esa decisión a **Gemini API multimodal** (`gemini-2.5-flash`) con ingestión directa del PDF como `inline_data` — sin parser server-side. Las menciones a *Ollama*, *gpt-oss:20b*, *OLLAMA_BASE_URL/MODEL*, *Smalot/PDFParser* y *cap de 6 000 chars* son **legado documental**: lo que se implementó vive en **ADR-07 ampliado** (sección "Detalles de implementación"), `backend/API/services/GeminiClient.php` y `backend/API/services/AIClient.php`. El §10 sigue siendo el punto de entrada útil para entender qué se difirió, ahora ya cerrado.
