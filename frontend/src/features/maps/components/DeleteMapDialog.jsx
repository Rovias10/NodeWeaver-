 ...
 * de modales. Defendible: API estándar del navegador, gestiona focus
 * trap y backdrop por sí sola.
 *
 * Props:
 *   - open:       bool (controla showModal/close).
 *   - mapTitle:   título a mostrar en el mensaje de confirmación.
 *   - isDeleting: muestra spinner en el botón "Eliminar".
 *   - onConfirm:  callback al confirmar.
 *   - onCancel:   callback al cancelar (botón Cancelar o Escape).
 */
export function DeleteMapDialog({ open, mapTitle = '', isDeleting = false, onConfirm, onCancel }) {
  ...
  return (
    <dialog
      ...
      "
      onClick={(e) => {
        // Click en el backdrop cierra (target = el propio dialog cuando
        // clicas fuera del contenido).
      ...
      <div className="p-6 md:p-7">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h2 className="text-lg font-bold text-ink">¿Eliminar este mapa?</h2>
            <p className="text-sm text-ink-muted mt-2">
              Vas a borrar <strong className="text-ink">«{mapTitle || 'sin título'}»</strong>
